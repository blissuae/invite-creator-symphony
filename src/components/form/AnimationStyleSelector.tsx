...
export const AnimationStyleSelector = ({
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {ANIMATION_STYLES.map(style => <div key={style.id} onClick={() => handleStyleSelection(style.id)} className="flex flex-col items-center space-y-4 cursor-pointer">
            <div className={`relative w-40 h-40 rounded-lg overflow-hidden transition-all ${selected.includes(style.id) ? "ring-4 ring-elegant-primary ring-offset-4" : "hover:ring-2 hover:ring-elegant-secondary hover:ring-offset-2"}`}>
              <img src={style.imageUrl} alt={style.title} className="w-full h-full object-cover" />
              <div className={`absolute inset-0 bg-elegant-primary/20 transition-opacity ${selected.includes(style.id) ? "opacity-70" : "opacity-0 group-hover:opacity-40"}`} />
              {selected.includes(style.id) && <div className="absolute top-4 right-4 bg-elegant-primary rounded-full p-1">
